...
def get_features_tfidf(Xr_fit, Xr_pred=None):
    ...
    vectorizer = TfidfVectorizer(analyzer=tokenise_text) # Set the analyzer argument to tokenise_text
    # Transform it into sparse matrix of TF-IDF features
...
def document_to_vector(tokenised_doc, word_vectors):
    ...
    # Initialize a list for vectors
    vectors = []

    for sentence in tokenised_doc:
        for token in sentence:
            # Append current token's vector if it is in word_vectord
            if token in word_vectors:
                vectors.append(word_vectors[token])

    # If no vectors were found, return zero vector
    if not vectors:
        return np.zeros(word_vectors.vector_size)

    # Aggregate the word vectors by taking the mean
