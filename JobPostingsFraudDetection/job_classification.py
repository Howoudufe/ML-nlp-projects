...
def prepare_dataset(filename):
    ...
    # 1. Education level task====================================
    print("Initial education level distribution:")
    print(df['required_education'].value_counts())

    # 1. Education level task====================================================
    # Only include specific education levels
    valid_education_levels = ["Master's Degree", "Bachelor's Degree", "High School or equivalent"]
    df = df[df['required_education'].isin(valid_education_levels)]

    # 1. For Education level task: Print filtered dataset information=========
    print(f"\nNumber of job postings after filtering: {len(df)}")
    print("Filtered education level distribution:")
    print(df['required_education'].value_counts())

    # 2. For Fraude task=========================
    # print(f"Total number of job postings: {len(df)}")
    # print("Fraudulent job posting distribution:")
    ...
    # 1. For education level task=======================
    for i, education_level in enumerate(le.classes_):
        print(f"Class {i} ({education_level}): {sum(y == i)}")

    # 2. For fraud task ==================
    # print(f"Real job postings: {sum(y == 0)}")
    # print(f"Fraudulent job postings: {sum(y == 1)}")
...
if __name__ == '__main__':
    ...
    # For education level task (TF-IDF)================================================
    ...
    # For fruad detection task (TF-IDF)==============================
    # Xr_train_fraud, y_train_fraud, Xr_val_fraud, y_val_fraud, Xr_test_fraud, y_test_fraud = prepare_dataset(filename=data_file)
    # acc_fraud = analyse_classification_tfidf(Xr_train_fraud, y_train_fraud, Xr_val_fraud, y_val_fraud, Xr_test_fraud, y_test_fraud)
    # print(f'Accuracy on test set for fraud detection (TF-IDF): {acc_fraud:.4f}')
    #==============================================================

    # uncomment to perform classification on the test set using aggregated word vectors

    # For education level task (w2v)========================
    # Xr_train, y_train, Xr_val, y_val, Xr_test, y_test = prepare_dataset(filename=data_file)
    ...
    # For fraud task (w2v)========================
    Xr_train_fraud, y_train_fraud, Xr_val_fraud, y_val_fraud, Xr_test_fraud, y_test_fraud = prepare_dataset(filename=data_file)
    acc_fraud = analyse_classification_w2v(Xr_train_fraud, y_train_fraud, Xr_val_fraud, y_val_fraud, Xr_test_fraud, y_test_fraud)
    print(f'Accuracy on test set for fraud detection (word2vec): {acc_fraud:.4f}')
